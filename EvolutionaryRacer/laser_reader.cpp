...
int main(int argc, char **argv)
{
    ...
    auto sensor_msg_shm_q = shmmap<Laser2dMsg<LASER_MSG_CAP>, SHM_QUEUE_SIZE>("laser_msgs"); // shared memory object
    assert(sensor_msg_shm_q);

    auto reader = sensor_msg_shm_q->getReader();

    std::vector<Laser2dMsg<LASER_MSG_CAP>> sensor_hits;
    sensor_hits.reserve(LASER_MSG_CAP);
    while (!WindowShouldClose())
    {
        Laser2dMsg<LASER_MSG_CAP> *msg = reader.readLast();
        if (!msg)
        {
        ...
        for (size_t i = 0; i < msg->size; ++i)
        {
            std::cout << "{" << msg->data[i].x << " " << msg->data[i].y << "}";
